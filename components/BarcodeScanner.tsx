"use client"
import { CameraDevice, Html5Qrcode, Html5QrcodeCameraScanConfig, Html5QrcodeFullConfig, Html5QrcodeResult, Html5QrcodeSupportedFormats} from "html5-qrcode";
...
type BarcodeScannerProps = {
  onScanSuccess: (decodedISBN: string, decodedResult: Html5QrcodeResult) => Promise<void>,
...
export default function BarcodeScanner({ onScanSuccess }: BarcodeScannerProps) {
  ...
  useEffect(() => {
    ...
    return (() => {
      if (scannerRef.current && scannerRef.current.isScanning) scannerRef.current.stop().then(() => console.log("Camera stopped"))
    })
  }, [])
  ...
  const startCamera = async (id:string) => {
    ...
    await camera.start(
      ...
      (decodedText, _) => {onScanSuccess(decodedText, _); scannerRef.current?.stop().then(() => {})},
      () => {}
    )
  }
  ...
  return (
    <div className="flex flex-col">
     {!scannerUp && (
      <div className="w-100 h-75 bg-gray-400 ">
        {cameras?.length && (
          <div className="rounded bg-blue-600 text-white">
            <div key={cameras[0].id} onClick={() => startCamera(cameras[0].id)}>Kuvaa viivakoodi (Kamera {`#${1}`})</div>
          </div>
        )}
      </div>
     )}
