...
import { gBooksApiResponseSchema, gBookSearchItemSchema, GoogleBookSearchItem } from "@/lib/validations"

export async function findBookByISBN(title: string, author: string): Promise<GoogleBookSearchItem[]> {
  try {
    const apiKey = process.env.GOOGLE_BOOKS_API_KEY;
    const q = encodeURIComponent(`${author}+intitle:${title}`)
